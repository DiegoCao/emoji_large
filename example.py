...
if __name__ == "__main__":
    ...
    sample_file = ["./data/2018-01-01-0.json.gz", "./data/2018-01-01-1.json.gz"]
    all_file = "./data/2018-01*.json.gz"

    df = spark.read.json(all_file)
    
    spark.sparkContext.setLogLevel("ERROR")
    dtypeframe = df.select('type')
    
    
    # print(type(dtypeframe))
    # rdd = df.rdd.map()
    # dtypeframe.groupby('type').count().show()
    
    # drepo = df.select('repo').count()
    # print('the total repo number is ', drepo)
    ...
    # ITERNUM =  200000
    # newdf = df.select('actor', 'payload')
    # aid = newdf.groupby('actor').count().show()
